...
--local utils = require("application.library.utils")
local config = {
    mysql = {
        timeout = 25000, --链接数据库超时时间15秒
        connect_config = {
            host = "127.0.0.1",
            port = 3306,
            database = "test",
...
function DB:parse_sql(sql, params)
    if not params or not self:table_is_array(params) or #params == 0 then
        return sql
    end
    local new_params = {}
